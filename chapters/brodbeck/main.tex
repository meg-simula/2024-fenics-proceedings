...
FEniCS is currently well-suited for handling residual-based error estimators, error estimates proposed by \cite{Bulle_BankWeiserApeFenics_2021} based on the strategy of Bank and Weiser, or automated, goal-oriented strategies introduced by \cite{Rognes_AutomatedEE_2013}, making it a valuable tool for many adaptive finite element methods. 
However, a posteriori error estimates involved in the equilibration of flux or stress, leading to guaranteed, fully localised and easily computable error upper bounds, especially in the energy norm, are not yet available. 
...
Following the philosophy of the FEniCS project, performance-relevant routines are written in C++ and can be used in Python via the appropriate bindings. 
...
\textbf{Equilibration in the presence of the full gradient} starts with the Poisson problem
\begin{equation}
...
\begin{definition}
    An equilibrated flux $\fluxR\in \Sigma(\Omega)$, with $\VecSpace[\domain]{\Sigma} := \{\vec{v} \in \HdivSpace[\domain]:\, \vec{v}\cdot\vec{n} = 0 \;\text{on}\; \Gamma_\mathrm{N} \}$ being the space of functions in H(div) with zero trace on the Neumann boundary $\Gamma_\mathrm{N}$, satisfies
    \begin{equation}
...
Further defining the Raviart--Thomas space of order $m$, $\feRT[m]$, one finds an upper bound on the error in a scaled $\mathrm{H}^1$ norm to hold.
\begin{theorem}
...
\vspace{-0.25cm}
\begin{theorem}
    Let $\Displacement \in \HiSpace[\domain][\Gamma_\mathrm{D}][2]$ be the solution of \eqref{eq:linear_elasticity-weak}, $\Displacement_\h \in \left(\VecSpace[]{V}_{\Gamma ,k}\right)^2$ be arbitrary, and $\LinPKEqlbH$ be an equilibrated stress. For $\delta\Displacement = \Displacement - \Displacement_\h$, one has
    \begin{equation}
...
\begin{proof}
    ...
    Integration by parts under the assumption of symmetry of the true stress and the equilibration conditions \eqref{eq:stress_eqlb_conditions} allows for reformulation of the mixed term:
    \begin{equation}
...
To keep the presentation general, in the following $\bm{\theta}$ denotes either a flux or a stress. 
...
\end{enumerate}
Therefore, the constrained minimization problem
\begin{equation}
...
Since a global solution of \eqref{eq:eqlb_by_global_minimisation} is computationally too expensive, the problem is localized by introducing for each node $z$ the nodal, piece-wise linear basis function $\hatfunc$. 
...
Algorithm \ref{alg:eqlb_mesh_level} describes the structure therefore required on the mesh level.
...
\textbf{Flux equilibration} requires the solution of a series of constrained minimization problems \eqref{eq:eqlb_by_local_minimisation}.
This can be done directly, as in \cite{Ern_FluxEqlb_2015}, or by splitting the process into an explicit part followed by an unconstrained minimization \cite[Appendix A]{Bertrand_HHO_2023}. 
Restricting this discussion to the second approach, the difference of the equilibrated and approximated flux is calculated in two steps:
\begin{equation}
...
\end{equation*} 
from the unconstrained minimization problem
\begin{equation}
...
Besides patches attached to the boundary $\Gamma_\mathrm{N}$, this allows one to assemble the system matrix $\mat{A}$ and compute its Cholesky decomposition only once per patch and reuse it for the different RHSs. 
...
\begin{algorithm}
    ...
    \For{$i = 0;\ i < n_\mathrm{RHS};\ i++$}
    {
        ...
        \uIf{$i = 0$}
        {
            Assemble $\mat{A}$ and $\vec{L}$ simultaneously and factorize $\mat{A}$\;
        }
        \Else
        {
...
Solving $\mat{S}\cdot\vec{c}=\vec{L}_c$ based on its lower--upper decomposition followed by solving $\mat{A}\cdot\vec{u}_{Ri}=-\mat{B}_i\cdot\vec{c}$ provides a notable speedup compared to a direct solution of \eqref{eq:eqs_ws_constrmin}. 
...
\begin{algorithm}
    ...
    \For{$i = 0;\ i < 2;\ i++$}
    {
        \lIf{$\left(\partial\patch \cap \Gamma_\mathrm{N} \neq \emptyset\right)$}
        {
            Apply BCs and refactorize $\mat{A}$
        }
...
The performance of an error estimate $\eta$ is characterized based on the efficiency index $ \mathrm{i_{eff}} = \eta/\mathrm{err}$, where $\mathrm{err}$ denotes the true error.
...
Convergence orders (e.o.c) and efficiency indices after the final refinement step are reported in Fig. \ref{fig:poisson-riviere_results}, while the final meshes for the first- and second-order approximations of the case $\kappa_1=100$ are shown in Figs. \ref{fig:poisson-riviere_refmesh-p1} and \ref{fig:poisson-riviere_refmesh-p2}, respectively. 
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Results of adaptive finite element method calculations with different orders $k$ and $m$. The e.o.c and $\mathrm{i_{eff}}$ values after the final refinement step are reported in (a).The two final meshes for $\kappa_1=100$ are depicted for $k=m-1=1$ in (b) and for $k=m-1=2$ in (c).}
...
Using Cooks membrane in Fig. \ref{fig:cook_definition}, this analysis is extended to linear elasticity, where the symmetry of the stress tensor is considered in a weak sense.
This analysis considers $\Pi_1=2.333$, $t=0.03$, and adaptive meshes based on a Dörfler marking strategy with $\theta=0.6$.
The characteristics of the first mesh satisfying $\vert\vert\vert \Displacement - \Displacement_\h \vert\vert\vert \leq 10^{-3}$ are summarized in Fig. \ref{fig:cook_results-summary}. Since $\LinPKEqlbH$ exactly fulfils the divergence condition from Definition \ref{def:equilibrated_stress}, $\eta$ is reduced to the sum of the $\mathcal{A}$-norm of the stress difference $\LinPKEqlbH - \stressPKLin_\h$ and the norm of asymmetric part of $\LinPKEqlbH$. 
...
Increasing the accuracy of the error estimate affects the effectiveness of the adaptive solution procedure -- measured by the number of degrees of
freedom required for a certain error -- in a positive way.
This trend is clearly much more pronounced for $k=2$, whereby similar accuracy is achieved with $47\%$ fewer degrees of freedom.
A practical shortcut -- since equilibration for $m=k+1$ is significantly more expensive than for $m=k$ -- appears to be the heuristic error indicator
\begin{eqnarray}
    \eta = \norm{\LinPKEqlbH - \stressPKLin_\h}\; ,
    \label{eq:elasticity_heuristic-ei}
\end{eqnarray}
where no weak symmetry is enforced on $\LinPKEqlbH$.
This yields efficiency indices close to one (see Fig. \ref{fig:cook_results-summary}) and, comparing the convergence history in Fig. \ref{fig:cook_results-convhist}, yields slightly better results as the guaranteed estimate with $m=k+1$.
\begin{figure}
    \centering
...
Comparing in a first step the relative equilibration costs $t_\mathrm{eqlb}/t_\mathrm{tot}$ for Cook's membrane with fixed meshes in Fig. \ref{fig:cook_performance-uniform}, one finds that -- for a primal problem of sufficient size -- $t_\mathrm{eqlb}$ is considerably smaller than $t_\mathrm{prime}$.  
Increasing $k$ increases the effort required for equilibration, while an equilibration with $m=k+1$ is significantly more expensive than the respective lowest-order case $m=k$. Similar timings for the adaptive solution of Cook's membrane (timings are accumulated until $\vert\vert\vert \Displacement - \Displacement_\h \vert\vert\vert \leq 10^{-3}$) are summarized in Fig. \ref{fig:cook_performance-adaptive}.
...
\section*{Conclusions}
This dolfinx\_eqlb contribution has introduced a FEniCSx-based library for the efficient equilibration of fluxes and stresses. 
...
In future work, we must still prove the efficiency of the implementation and the heuristic error indicator for real-world problems. 
We further intend to generalize the implementation to 3D domains and multiphysics problems such as poroelasticity.
\vspace{-0.1cm}
\subsection*{Supplementary material}
This work is based on dolfinx\_eqlb v1.2.0 (\url{https://github.com/brodbeck-m/dolfinx_eqlb/tree/v1.2.0}). 
The examples presented can be accessed via either GitHub (\url{https://github.com/brodbeck-m/AFEM-by-Equilibration}) or, for a Docker image, \cite{DatsetPaper}.
\vspace{-0.1cm}
