...
\institute{ 
    Maximilian Brodbeck \email{brodbeck@isd.uni-stuttgart.de} \at Institute of Structural Mechanics and Dynamics, University of Stuttgart, Stuttgart, Germany \\
    \and 
...
However, a posteriori error estimates based on the equilibration of flux or stress leading to guaranteed, fully localised and easily computable error upper bounds, especially in the energy norm, are not yet available.
...
\end{equation}
holds. The differences $\delta\flux$ and $\delta\GenSol$ denote $\flux(\GenSol) - \fluxR$ and $\GenSol - \GenSolH$, respectively. Further introducing the Raviart--Thomas space of order $m$, $\feRT[m]$, one finds an upper bound on the error in a scaled $\mathrm{H}^1$ norm to hold.
\begin{theorem}
    Let $\kappa$ be piecewise constant, $\GenSol \in \HiSpace[\domain][\Gamma_\mathrm{D}]$ be the solution of \cref{eq:poisson_weak}, $\GenSolH \in \VecSpace[]{V}_{\Gamma ,k}$ be arbitrary, and $\fluxRH\in\feRT[m]$ be an equilibrated flux. Then
    \begin{equation}
...
\vspace{-0.25cm}
\begin{theorem}
    Let $\Displacement \in \HiSpace[\domain][\Gamma_\mathrm{D}][2]$ be the solution of \cref{eq:linear_elasticity-weak}, $\Displacement_\h \in \left(\VecSpace[]{V}_{\Gamma ,k}\right)^2$ be arbitrary, and $\LinPKEqlbH$ be an equilibrated stress. For $\delta\Displacement = \Displacement - \Displacement_\h$ it holds
    \begin{equation}
...
\begin{proof}
    ...
    Integration by parts considering the symmetry of the true stress and the equilibration conditions \cref{eq:stress_eqlb_conditions} allows a reformulation of the mixed term:
    \begin{equation}
    ...
    \end{equation}
    Based on the weak symmetry, \cref{eq:proof_ee-le_reformulation} can be bounded from above
    \begin{equation}
    ...
    \end{equation}
    Inserting \cref{eq:proof_ee-le_bound_one} into \cref{eq:proof_ee-le_eval_anorm} completes the proof.
\end{proof}
...
Since a global solution of \cref{eq:eqlb_by_global_minimisation} is computationally too expensive, the problem is localised by introducing for each node $z$ the nodal, piece-wise linear basis function $\hatfunc$. 
...
The following discusses the patch-local algorithms for flux equilibration \cref{alg:eqlb_se_patch_level} and enforcement of the weak symmetry condition \cref{alg:eqlb_ws_patch_level}.
\begin{algorithm}
...
\textbf{Flux equilibration} requires the solution of a series of constrained minimisation problems \cref{eq:eqlb_by_local_minimisation}.
This can be done directly\citep{Ern_FluxEqlb_2015} or by splitting the process into an explicit part followed by an unconstrained minimisation \cite[Appendix A]{Bertrand_HHO_2023}. 
...
\cite{Bertrand_EqlbElast_2021} have proven the solvability of \cref{eq:eqlb_ws_constrmin} when patches have at least two internal facets and $k,m \geq 2$.
...
\end{equation}
the equation system resulting from the saddle point problem in \cref{eq:eqlb_ws_constrmin}, a Schur
complement-based solver, is implemented. The vectors $\vec{u}_{R1}$ and $\vec{u}_{R2}$ denote the degrees of freedom (DOFs) for the rows of the stress tensor.
...
Solving $\mat{S}\cdot\vec{c}=\vec{L}_c$ based on its LU decomposition followed by solving $\mat{A}\cdot\vec{u}_{Ri}=-\mat{B}_i\cdot\vec{c}$ is notably faster than a direct solution of \cref{eq:eqs_ws_constrmin}. 
...
The efficiency of the error estimate \cref{eq:ee_poisson-disc-kappa} depends on the degree $m$ of the equilibrated flux.
...
Using the Cook's membrane in \cref{fig:cook_definition}, this analysis is extended to linear elasticity, where the symmetry of the stress tensor is considered in a weak sense.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.56\textwidth}
        \centering
        \includegraphics[scale=0.95]{fig_cook-convhistory.pdf}
        \caption{}
        \label{fig:cook_results-convhist}
    \end{subfigure}
    \caption{Effectivity of the different adaptive solution procedures for Cook's membrane: (a) summarises the results for the first mesh with $\mathrm{err} = \vert\vert\vert \Displacement - \Displacement_\h \vert\vert\vert \leq 10^{-3}$, (b) details the convergence history (black, $k=2$; blue, $k=3$). Orders $m^*$ indicate the use of \cref{eq:elasticity_heuristic-ei}.}
...
As for $k=2$, the heuristic indicator \cref{eq:elasticity_heuristic-ei} with $m=k$ performs best, and the higher-order estimate with $m=k+1$ is outperformed.
