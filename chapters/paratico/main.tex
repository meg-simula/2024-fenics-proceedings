...
\author{Sara Paratico, Riccardo Munaf\`o, Chiara Trenti, Petter Dyverfeldt, Simone Saitta, and Emiliano Votta}
\authorrunning{Paratico et al.}

\institute{
    Sara Paratico \email{sara.paratico@mail.polimi.it} \at Politecnico di Milano, Milan, Italy
    \and
    Riccardo Munaf\`o \at Politecnico di Milano, Milan, Italy
    \and 
    Chiara Trenti \at Division of Cardiovascular Medicine, Department of Health, Medicine and Caring Sciences, Linköping University, Universitetssjukhuset, 581 83 Linköping, Sweden and \\ Center for Medical Image Science and Visualization (CMIV), Linköping University, Universitetssjukhuset, 581 83 Linköping, Sweden
    \and
    Petter Dyverfeldt \at Division of Cardiovascular Medicine, Department of Health, Medicine and Caring Sciences, Linköping University, Universitetssjukhuset, 581 83 Linköping, Sweden and \\ Center for Medical Image Science and Visualization (CMIV), Linköping University, Universitetssjukhuset, 581 83 Linköping, Sweden
    \and
    Simone Saitta \at Politecnico di Milano, Department of Electronics, Information, and Bioengineering, Milan, Italy
    \and 
    Emiliano Votta \at Politecnico di Milano, Department of Electronics, Information, and Bioengineering, Milan, Italy
}
\maketitle
...
The VarDA approach was formulated as an optimisation problem constrained by the NS equations, using the dolfin-adjoint library for the adjoint problem. The process follows three steps: running a first numerical simulation with tentative inlet BCs (which we refer to as the \emph{forward model} or \emph{tape}), solving the optimisation problem to identify inlet BCs, and running a final numerical simulation yielding the refined velocity and pressure fields (\cref{fig:scheme}).
...
\subsection{Optimisation Problem Definition}

The optimisation problem \cref{eq:10}, constrained by the NS equations, aims to minimise a functional $J(\mathbf{u})$ \cref{eq:11}, defined as the difference between the computed and observed velocities:
...
The adjoint approach efficiently computes the total derivative of the functional, yielding the adjoint NS equations that facilitate optimisation. The iterative Broyden--Fletcher--Goldfarb--Shanno (BFGS) algorithm, in its L-BFGS variant \citep{Liu1989}, serves as an optimiser. The L-BFGS algorithm is already implemented in the SciPy library, which is automatically
called by the dolfin-adjoint library and provides many user-friendly numerical routines, such as the routine for optimisation.
...
\begin{figure}
    ...
    Centre: Discrepancy between the CFD velocity field and experimental data is minimised by iteratively refining inlet velocity profile with a gradient-based method.}
    \label{fig:scheme}
\end{figure}
...
The performance of the method was assessed by the $J(\mathbf{u})$ values before and after optimisation, the root mean squared error (RMSE) between \( \mathbf{u}\) and \( \mathbf{u}_{\text{obs}} \), and qualitative analysis of the effect on the velocity field through the software ParaView.
...
Iterative minimisation of the discrepancy between \( \mathbf{u} \) and \( \mathbf{u}_{\text{obs}} \) was carried out to determine the optimal velocity profile for CFD simulations, verifying that it matches the parabolic profile used to generate the synthetic observations.\\


\paragraph{Progressively Demanding Tests.}
...
In high--Reynolds number tests, IPCS optimisation reduced the RMSE from 142.60 mm/s to 6.70 mm/s, while the coupled scheme faced convergence issues. Under low-Reynolds number conditions, the IPCS proved to be five times faster than the coupled scheme and achieved a final RMSE of 1.76 mm/s, compared to 4.22 mm/s for the coupled scheme.

\subsection{Progressively Demanding Tests}
\paragraph{2DVar+t Benchmark.}
When a zero-velocity field was imposed as an IC, the post-optimisation velocity field showed inconsistencies with respect to the observations.
...
Generating a tape took about 25 minutes, while optimisation required 17 hours with 80 Gb of memory.
...
\section{Conclusions}
This study applies VarDA to estimate optimal inlet BCs for CFD using noisy 4D flow MRI data, minimising mismatches with in vivo velocity measurements.
The method yields a resolved, noise-free velocity field and has been validated on 2D, 3D, and patient-specific AAA geometries, demonstrating potential for personalised haemodynamic simulations.
