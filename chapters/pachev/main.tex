...
\institute{
  Benjamin.~A.~Pachev \email{benjaminpachev@utexas.edu} \at The University of Texas at Austin, Austin, Texas, United States
  \and
  James.~D.~Trotter \email{james@simula.no} \at Simula Research Laboratory, Oslo, Norway
  \and
...
We begin by initialising the PETSc data structures needed for assembly and linear algebra.
\begin{python}
...
if use_cuda:
  ...
  ksp.setOperators(A)
\end{python}
Finally, we reach the Newton iteration loop. Note the use of PETSc operations to add the computed Newton update to the solution, instead of vectorised NumPy operations as is common in FEniCSx code.
...
\end{table}
Overall, the CUDA-accelerated solver averaged 2.07\,s per time step, while the original solver took 7.89\,s per time step. The overall speedup is therefore a factor of 
3.8${\times}$. While the solution time is dominated by the linear solve for the first stage, assembly comprises over 10\% of the total runtime for both the GPU and CPU codes. 
