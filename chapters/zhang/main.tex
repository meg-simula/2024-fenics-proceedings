...
\institute{Yanjun Zhang \email{yanjunzh@kth.se} \at KTH Royal Institute of Technology, Stockholm, Sweden }
...
The test rig mainly consists of a DC motor, flywheels, a brake pad, and a brake disc, as shown in Fig. \ref{fig:test rig}. The maximum motor power is 450 kW, and the maximum motor torque is 4,000 Nm. The brake pressure at the reservoir ranges from zero to 10 bar. 
...
The first step is mesh sensitivity and time step analysis, where we aim to show that the simulation results converge with finer mesh sizes and smaller time steps. Since no exact solution exists, the average temperature of point T1, as shown in Fig. \ref{fig:thermocouples}, is used as the convergence parameter.
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{chapters/zhang/graphics/ave_T_vs_dof.pdf}

    \caption{Convergence test, with average temperatures of point T1 against degrees of freedom (DOFs)}
    \label{fig:error_mesh}
\end{figure}

As shown in Fig. \ref{fig:error_mesh}, the average temperature of point T1 increases with more DOFs, up to 1 million DOFs. Above 1 million, however, the temperature remains constant.  These results show that above 1 million DOFs is sufficient to capture the characteristics of the system. 
\begin{figure}[h]
    \centering
  
    \includegraphics[width=0.8\textwidth]{chapters/zhang/graphics/T_ave_vs_dt.pdf}

    \caption{Convergence test, with average temperatures of point T1 against time steps}
    \label{fig:error_time}
\end{figure}


Figure \ref{fig:error_time} compares the average temperatures of point 1 and times steps. When \(dt\) is over 1 s, the temperature has great variance. The best value of \(dt\) is 0.16 s (point at 148$^{\circ}\text{C}$ ), which is a balanced time step between accuracy and computation time.
...
This work was sponsored by the KTH Railway Group, the China Scholarship Council, and CRRC ZELC. We are grateful for the experimental support of Fei Gao and Junying Yang at Dalian Jiaotong University. We especially acknowledge the help of Jørgen S. Dokken from the FEniCSx community and the support of Jing Gong from Kungliga Tekniska Högskolans PDC. We also thank the National Academic Infrastructure for Supercomputing in Sweden for providing computer resources.        
\end{acknowledgement}
