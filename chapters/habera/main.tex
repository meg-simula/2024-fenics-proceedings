...
\institute{M.~Habera \email{michal.habera@rafinex.com} \at Rafinex SARL, and Institute of Computational Engineering, Department of Engineering, Faculty of Science, Technology and Medicine, University of Luxembourg\\
...
Figure \ref{fig:stream-single} shows the results for the copy operation for a single-node
...
$n = 165$ for eighth-order discretization (high-order) discretization. Low-order kernels are
...
is required to understand the potential and determine missed optimizations.
...
loop body. For the eight order Laplace operator, \lstinline{NUM_QUAD_POINTS = 214} and \lstinline{NUM_DOFS = 165}. There are two
...
The kernel benchmark results are presented in Figs. \ref{fig:local-deg1}
and \ref{fig:local-deg8}. Low-order kernels (Fig. \ref{fig:local-deg1}) show no
...
High-order kernels (Fig. \ref{fig:local-deg8}), which are expected to benefit from
...
speedup (\textgreater $2\times$) in the most optimized setting. The vectorization
...
per process) are shown in Fig. \ref{fig:weak-scaling}. Both Aion and AWS c7gn
