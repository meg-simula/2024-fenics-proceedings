...
instruction set with Scalable Vector Extension (SVE) modules for Single Instruction
...
optimized code using SVE instructions, ensuring that users can achieve
...
Services, or AWS (Graviton3 CPU based on Neoverse V1 and Graviton4 CPU with
...
One advantage of a compute-bound kernel is that it ensures that the
assembly code of the compiled kernel contains calls to SIMD operations. SIMD operations can apply the same operation
...
amenable to the compiler applying automatic vectorization, a process that
automatically converts a scalar program into a vectorized equivalent that uses
SIMD operations.
...
In addition to SIMD parallelization at the kernel level, DOLFINx, the FEniCS Project's finite
element problem solving environment, also supports
the distributed memory parallel assembly of global finite element data structures
...
\begin{table}
  ...
  \begin{tabular}{l|l|l}
    ...
    Memory                    & \makecell[l]{256 GB DDR4 \\ 3200 MT/s = 25.6 GB/s \\ 8 non-uniform memory access (NUMA) nodes} & \makecell[l]{128 GB DDR5 \\ 4800 MT/s = 38.4 GB/s  \\ Unified memory access (no NUMA) } \\ \hline
...
\autoref{fig:stream-single} shows the results for the copy operation for a single-node
benchmark. For the single-node benchmark, $\SI{80}{\percent}$ of the
...
saturation is observed at around $\SI{20}{\percent}$ of the node utilization.
Both curves show different saturation point characteristics due to
different memory access configurations. On the Aion instances, there are eight NUMA nodes of 16 cores each, while AWS c7g instances
are set up with unified memory access.
...
Compiler (loop) SIMD auto-vectorization is usually performed for innermost
...
\ref{lst:c-code}. First, there are arrays defining finite element basis
...
set of multiply--add operations that are candidates for automatic vectorization
...
The kernel benchmark results are presented in \autoref{fig:local-deg1}
and \autoref{fig:local-deg8}. Low-order kernels (\autoref{fig:local-deg1}) show no
dependence on the compiler vectorization setup. On the other hand, AWS c7g demonstrates a
1.3x speedup over Aion, which we attribute to greater memory bandwidth for single processes.
...
Both clang and GCC auto-vectorizers perform well, producing a noticeable
speedup (\textgreater 2x) in the most optimized setting. The vectorization
speedup (\textgreater 4x) is more significant with the Aion nodes.
...
\texttt{-fopt-info-vec-optimized} for GCC) and analysis of the generated
assembly reveal that, for low-order operator action, using the \lstinline{-Ofast} compiler optimization
...
compiler auto-vectorization. Moreover, for higher-order
discretization coefficients, there is potential for exploiting wider SVE registers (up to 2,048 bits).
...
of the vectorized loop (FADDV). Here P0 is a predicate register without any
constraints on the available elements.

On Aion, both GCC and clang vectorize both the coefficient evaluation and tensor
assignment loops and rely on the \lstinline{VFMADD231PD} instructions on the
YMM registers, that is, the vectorization width of four doubles
...
In terms of the auto-vectorization capabilities of GCC 13.2.0 and clang 18.1.3, both
produce optimized instructions for the targeted microarchitectures (Zen 2 for
...
~(\cite{VCPKVO_HPCCT22}; see \url{https://hpc.uni.lu}.

This research was funded in whole or in part by the National Research
Fund, grant reference COAT/17205623. For the purpose of open
