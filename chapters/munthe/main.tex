...
\eqref{eq: stress} still need to be specified. We will define the growth laws in the next section, but we use a nearly incompressible neo-Hookean model for all the 
experiments, so \eqref{eq: stress} becomes
\begin{align*}
...
\begin{algorithm} 
    ...
    \SetAlgoLined
    \For{each time step}{
        Solve \eqref{eq: system of equations} for the displacement $\mathbf{u}$\;
...
\end{align}
Since we are using test functions $\eta$ that vanish on $\partial_D\mathcal{M}$ and the normal component of $\mathbf{P}$ is zero on $\partial_N\mathcal{M}$, we can set the boundary integral to zero. Then \eqref{eq: weak conservation of momentum} is solved using FEniCSx \citep{DOLFINx}. 
\par
\emph{Iteratively solving the conservation of momentum}: We now solve \eqref{eq: weak conservation of momentum} for the displacement $\mathbf{u}$, which we can use to compute all the necessary variables. We use tetrahedral, second-order, continuous Lagrange elements to approximate $\mathbf{u}$, and first-order, discontinuous Lagrange elements to approximate $\mathbf{F}_e$ and $\mathbf{F}_g$. This is a common numerical scheme in cardiac mechanics that has been demonstrated to avoid locking \citep{oliveira2016comparison}. 
